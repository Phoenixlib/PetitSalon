  ...
  return (
    <>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold" style={{ color: "var(--ps-text)" }}>
            Servicios
          </h1>
          <p className="text-sm mt-0.5" style={{ color: "var(--ps-text-mid)" }}>
            {active.length} activo{active.length !== 1 ? "s" : ""} · {inactive.length} inactivo
            {inactive.length !== 1 ? "s" : ""}
          </p>
        </div>
        <button
      ...
      <div className="rounded-2xl overflow-hidden shadow-sm" style={{ border: "1px solid var(--border)" }}>
        <div className="px-5 py-3.5" style={{ backgroundColor: "var(--ps-lila-pale)", borderBottom: "1px solid var(--border)" }}>
          <h2 className="text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--ps-text-mid)" }}>
        ...
        {active.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 gap-3 bg-white">
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" style={{ color: "var(--ps-text-mid)", opacity: 0.4 }}>
      ...
      {/* Tabla inactivos */}
      {inactive.length > 0 && (
        <div className="mt-6 rounded-2xl overflow-hidden shadow-sm" style={{ border: "1px solid var(--border)" }}>
          <div className="px-5 py-3.5" style={{ backgroundColor: "#f5f5f5", borderBottom: "1px solid var(--border)" }}>
            <h2 className="text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--ps-text-mid)" }}>
  ...
  return (
    <div className="flex items-center gap-4 px-5 py-4">
      ...
      <div className="flex-1 min-w-0">
        ...
        {service.description && (
          <p className="text-xs truncate mt-0.5" style={{ color: "var(--ps-text-mid)" }}>
            {service.description}
          </p>
        )}
      </div>
