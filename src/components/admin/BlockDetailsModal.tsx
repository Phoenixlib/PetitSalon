  ...
  return (
    <div className={`${isFullscreen ? "absolute" : "fixed"} inset-0 z-[99999] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-300`}>
      <div className="relative bg-white w-full max-w-md rounded-3xl shadow-2xl border-t-[8px] border-red-500 overflow-hidden flex flex-col max-h-[90vh]">
        {showConfirm ? (
          <div className="p-8 text-center flex flex-col items-center justify-center space-y-4 animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-sm text-gray-600 leading-relaxed font-body">
              ¿Estás seguro de que deseas eliminar este bloqueo de horario? Esta acción no se puede deshacer y volverá a habilitar el horario en la agenda.
            </p>
            
            {errorMsg && (
              <div className="w-full rounded-xl bg-red-50 border border-red-100 p-3 text-sm text-red-600 font-bold font-body">
                {errorMsg}
              </div>
            )}

            <div className="flex gap-3 w-full pt-4 font-body">
              <button
                onClick={() => {
                  setShowConfirm(false);
                  setErrorMsg(null);
                ...
                disabled={isPending}
                className="flex-1 px-6 py-2.5 rounded-xl font-bold text-xs tracking-wider uppercase bg-gray-100 hover:bg-gray-200 text-gray-700 transition-colors disabled:opacity-50"
              >
                Cancelar
              </button>
              <button
                ...
                disabled={isPending}
                className="flex-1 px-6 py-2.5 rounded-xl font-bold text-xs tracking-wider uppercase bg-red-600 hover:bg-red-700 text-white transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
              >
          ...
          <>
            ...
            <div className="p-6 border-b border-gray-100 flex justify-between items-start bg-red-50 flex-shrink-0">
              <div className="flex gap-3">
                ...
                <div>
                  <h3 className="text-lg text-red-900 font-bold leading-none mb-1 font-title">
                    Horario Bloqueado
                  </h3>
                  <p className="text-xs text-red-700 font-medium max-w-[200px] font-body">
            ...
            {/* Footer */}
            <div className="p-4 border-t border-gray-100 bg-white flex gap-3 flex-shrink-0 justify-between items-center font-body">
              <button
