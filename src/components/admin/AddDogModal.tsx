...
import { addDogAction, DogFormState } from "@/app/admin/clientes/actions";
...
export default function AddDogModal({
  ...
  const [state, formAction, pending] = useActionState<DogFormState, FormData>(
    addDogAction.bind(null, ownerId),
    {}
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
      ...
      >
        <div className="flex items-center justify-between border-b px-6 py-4" style={{ backgroundColor: "var(--ps-lila-pale)", borderColor: "var(--border)" }}>
          <h2 className="text-xl font-bold" style={{ color: "var(--ps-text)" }}>Nueva Mascota</h2>
          ...
          <form action={formAction} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <input name="name" type="text" required className="w-full rounded-lg px-4 py-2 border focus:ring-2 focus:ring-[var(--primary)]" style={{ borderColor: "var(--border)" }} />
              ...
              <div className="space-y-1">
                ...
                <input name="breed" type="text" required className="w-full rounded-lg px-4 py-2 border focus:ring-2 focus:ring-[var(--primary)]" style={{ borderColor: "var(--border)" }} />
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="space-y-1">
                ...
                <input name="weight" type="text" placeholder="ej. 5 kg" className="w-full rounded-lg px-4 py-2 border focus:ring-2 focus:ring-[var(--primary)]" style={{ borderColor: "var(--border)" }} />
              </div>
            </div>

            <div className="space-y-1">
              <label className="text-sm font-medium" style={{ color: "var(--ps-text-mid)" }}>Notas / Observaciones</label>
              <textarea name="notes" rows={3} className="w-full rounded-lg px-4 py-2 border focus:ring-2 focus:ring-[var(--primary)]" style={{ borderColor: "var(--border)" }}></textarea>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t mt-6" style={{ borderColor: "var(--border)" }}>
              ...
              <button type="submit" disabled={pending} className="px-6 py-2 rounded-full text-sm font-bold text-white bg-[var(--primary)] hover:opacity-90 disabled:opacity-50 transition-opacity">
                {pending ? "Guardando..." : "Agregar Mascota"}
              </button>
            </div>
          </form>
        </div>
