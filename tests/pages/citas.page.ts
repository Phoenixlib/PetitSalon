...
export class CitasPage {
  ...
  constructor(private readonly page: Page) {
    this.estadoFiltro = page.getByRole('combobox', { name: /filtrar por estado/i });
  }
  ...
  async cambiarEstadoCita(index: number, nuevoEstado: string): Promise<void> {
    ...
    if (/confirmada|confirmar/i.test(nuevoEstado)) {
      ...
      const soloMarcarBtn = this.page.getByRole('button', { name: /solo marcar como realizado/i });
      await expect(soloMarcarBtn).toBeVisible({ timeout: 5000 });
      await soloMarcarBtn.click();
      // Handle the review-prompt step
      const noConfirmBtn = this.page.getByRole('button', { name: /no, solo marcar como realizado/i });
  ...
  async verBadgeEstado(index: number): Promise<Locator> {
    return this.page.getByTestId('cita-row').nth(index).getByTestId('estado-badge');
  }
}
